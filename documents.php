...
// Certificados ATIVOS — isolado por usuário
$documents = loadData('userCertificates', true);
if (empty($documents)) {
    $documents = [];
}

// Certificados na LIXEIRA — isolado por usuário
$trash = loadData('userCertificatesTrash', true);
if (empty($trash)) {
    $trash = [];
}

// ✅ NOVO: Certificados removidos definitivamente, mas mantidos "arquivados"
// Nunca aparecem na tela, mas continuam contando nas horas/total,
// pois o certificado é um registro OFICIAL da faculdade — o app só
...
// Mover certificado para a lixeira (não apaga de verdade)
...
// ✅ ALTERADO: "Excluir definitivamente" agora move o item para o
...
// mesmo removendo da lixeira, pois o registro é oficial da faculdade.
...
// ✅ ALTERADO: total de horas e total de certificados agora somam
// ativos + lixeira + arquivo morto — nada reduz o total, mesmo excluindo
    ...
    <div class="p-8 max-w-7xl mx-auto">
        ...
        <div id="modalExcluir" class="fixed inset-0 z-[999] hidden items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 z-10">
                ...
                <p class="text-gray-400 text-xs text-center mb-8">O certificado irá para a lixeira e poderá ser restaurado depois. As horas continuam sendo computadas, pois o registro é oficial da faculdade.</p>
            ...
            <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8 z-10 max-h-[80vh] overflow-y-auto">
                ...
                <?php if (empty($trash)): ?>
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-400 text-sm">A lixeira está vazia.</p>
                    </div>
                <?php else: ?>
                    <div class="space-y-3">
                        <?php foreach ($trash as $doc): ?>
                            <div class="flex items-center justify-between gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-xs text-gray-500"><?php echo htmlspecialchars($doc['institution']); ?> · <?php echo htmlspecialchars($doc['hours']); ?></p>
                                    <p class="text-xs text-gray-400 mt-1">Excluído em <?php echo htmlspecialchars($doc['deletedAt'] ?? ''); ?></p>
                                </div>
        ...
        <div id="modalExcluirDefinitivo" class="fixed inset-0 z-[1000] hidden items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 z-10">
                ...
                <p class="text-gray-500 text-sm text-center mb-1 leading-relaxed">Você está prestes a remover da lixeira:</p>
                <p id="modalNomeExcluirDefinitivo" class="text-[#4A9FCA] font-semibold text-sm text-center mb-3 leading-relaxed"></p>
                <p class="text-gray-400 text-xs text-center mb-8">O certificado deixará de aparecer aqui, mas continuará computado nas suas horas, pois é um registro oficial da faculdade.</p>
