...
checkAdmin(); //bloqueia alunos comuns via servidor, não só via menu escondido
...
//Usa loadData() como o resto do sistema
$posts = loadData('platform_posts');
if (empty($posts)) {
    $posts = $initialPosts;
}

//Configurações de upload seguro (reaproveitadas no save e no edit)
$allowedExts = ['jpg', 'jpeg', 'png', 'webp', 'gif'];
$maxSize = 5 * 1024 * 1024; // 5MB por imagem

function salvarImagensPost($filesArray) {
    ...
    foreach ($filesArray['tmp_name'] as $key => $tmp_name) {
        if ($filesArray['error'][$key] !== 0) continue;

        $ext = strtolower(pathinfo($filesArray['name'][$key], PATHINFO_EXTENSION));
        $tamanhoOk = $filesArray['size'][$key] <= $maxSize;
        $isImagemReal = in_array($ext, $allowedExts) && @getimagesize($tmp_name) !== false;

        if (!$tamanhoOk || !$isImagemReal) continue; //ignora arquivos inválidos

        $fileName = time() . '_' . uniqid() . '.' . $ext;
        $uploadPath = $uploadDir . $fileName;
        if (move_uploaded_file($tmp_name, $uploadPath)) {
            $savedPaths[] = $uploadPath;
        }
    }
    return $savedPaths;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        if (!empty($_FILES['images']['name'][0])) {
            $newPost['images'] = salvarImagensPost($_FILES['images']);
        }
    ...
    if ($action === 'edit') {
        ...
        foreach ($posts as &$post) {
            if ($post['id'] == $id) {
                ...
                if (!empty($_FILES['images']['name'][0])) {
                    //apaga as imagens antigas do disco antes de substituir
                    foreach ($post['images'] as $oldImg) {
                        if (file_exists($oldImg)) unlink($oldImg);
                    }
                    $post['images'] = salvarImagensPost($_FILES['images']);
                }
                break;
            }
        }
    ...
    if ($action === 'delete') {
        ...
        //apaga as imagens do post do disco antes de remover o registro
        foreach ($posts as $post) {
            if ($post['id'] == $id) {
    ...
    <div class="p-8 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php foreach ($posts as $post):
                $imagensPost = !empty($post['images']) ? array_values($post['images']) : [];
                $totalSlidesPost = max(count($imagensPost), 1);
            ?>
...
<p class="text-gray-600 text-sm mb-6 line-clamp-3 leading-relaxed">
    <?php 
        ...
        echo htmlspecialchars(substr($conteudo, 0, 150)) . (strlen($conteudo) > 150 ? '...' : '');
    ?>
</p>
                        ...
                        <div class="flex gap-3 pt-6 border-t border-gray-100">
                            ...
                            <!-- Visualizar (preview inline = carrossel igual ao Dashboard) -->
                            <button type="button"
                                id="btnPreview_<?php echo $post['id']; ?>"
                                onclick='togglePreview(<?php echo $post["id"]; ?>)'
                                title="Ver como os alunos vão visualizar"
                                class="w-12 px-3 py-2 border border-gray-200 text-gray-600 rounded-xl text-sm font-semibold hover:bg-gray-50 transition-all flex items-center justify-center">
                        ...
                        <!-- =========================================================
                             âœ… PREVIEW INLINE = MESMO CARROSSEL DO DASHBOARD.PHP
                             (wrapper, setas no hover, bolinhas) â€” escondido por padrão
                        ========================================================== -->
                        <div id="previewBox_<?php echo $post['id']; ?>" class="hidden mt-6">

                            <p class="text-xs text-gray-400 mb-3 flex items-center gap-2">
                                <i class="fas fa-info-circle"></i>
                                Prévia exata de como este post aparece no Dashboard dos alunos:
                            </p>

                            <div id="postCarouselWrapper_<?php echo $post['id']; ?>" class="relative group">

                                <div class="overflow-hidden rounded-[24px]">
                                    <div id="postCarouselTrack_<?php echo $post['id']; ?>" class="flex transition-transform duration-500 ease-in-out">
                                        <?php if (!empty($imagensPost)): ?>
                                            <?php foreach ($imagensPost as $img): ?>
                                ...
                                <?php if ($totalSlidesPost > 1): ?>
                                    <!-- Setas de navegação — visíveis apenas ao passar o mouse sobre o carrossel -->
                                    <button type="button" onclick="moverCarrosselPreview(<?php echo $post['id']; ?>, -1)"
                                    ...
                                    <div class="flex justify-center gap-2 mt-4">
                                        <?php for ($i = 0; $i < $totalSlidesPost; $i++): ?>
                                            <button type="button" onclick="irParaSlidePreview(<?php echo $post['id']; ?>, <?php echo $i; ?>)"
                                                data-dot-preview="<?php echo $post['id']; ?>_<?php echo $i; ?>"
                                                class="w-2.5 h-2.5 rounded-full <?php echo $i === 0 ? 'bg-[#2B7FA6]' : 'bg-gray-300'; ?> transition-all"></button>
                                        <?php endfor; ?>
                                    </div>
                                <?php endif; ?>
            ...
            <?php if (empty($posts)): ?>
                <div class="col-span-full p-24 text-center border-2 border-dashed border-gray-200 bg-white rounded-[40px]">
                    ...
                    <h3 class="text-2xl font-bold text-gray-900 mb-2">Sua galeria estão vazia</h3>
        ...
        <!-- âœ… Modal Excluir â€” mesmo estilo do documents.php -->
...
<script>
    ...
    // =============================
    // Preview inline carrossel idêntico ao do dashboard.php
    // (cada post tem seu próprio estado de slide, guardado por id)
    // =============================
    const slideAtualPreview = {};

    function togglePreview(id) {
        const box = document.getElementById('previewBox_' + id);
        const btn = document.getElementById('btnPreview_' + id);

        const estaAberto = !box.classList.contains('hidden');

        if (estaAberto) {
            box.classList.add('hidden');
            btn.classList.remove('bg-[#4A9FCA]', 'text-white', 'border-[#4A9FCA]');
        } else {
            box.classList.remove('hidden');
            btn.classList.add('bg-[#4A9FCA]', 'text-white', 'border-[#4A9FCA]');
            if (slideAtualPreview[id] === undefined) slideAtualPreview[id] = 0;
            atualizarCarrosselPreview(id);
            setTimeout(() => box.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 100);
        }
    }

    function atualizarCarrosselPreview(id) {
        const track = document.getElementById('postCarouselTrack_' + id);
        if (!track) return;
        const slide = slideAtualPreview[id] || 0;
        track.style.transform = `translateX(-${slide * 100}%)`;

        document.querySelectorAll(`[data-dot-preview^="${id}_"]`).forEach(dot => {
            const idx = parseInt(dot.dataset.dotPreview.split('_')[1]);
            dot.classList.toggle('bg-[#2B7FA6]', idx === slide);
            dot.classList.toggle('bg-gray-300', idx !== slide);
        });
    }

    function moverCarrosselPreview(id, direcao) {
        const track = document.getElementById('postCarouselTrack_' + id);
        if (!track) return;
        const totalSlides = track.children.length;
        slideAtualPreview[id] = ((slideAtualPreview[id] || 0) + direcao + totalSlides) % totalSlides;
        atualizarCarrosselPreview(id);
    }

    function irParaSlidePreview(id, indice) {
        slideAtualPreview[id] = indice;
        atualizarCarrosselPreview(id);
    }
