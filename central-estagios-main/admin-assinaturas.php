...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header('Location: admin-assinaturas.php');
    exit;
}
...
<style>
    ...
    /* Lista de arquivos selecionados (aparece abaixo da area unica) */
    .upload-file-list {
        margin-top: 0.85rem;
        display: flex;
...
<div class="p-8 max-w-6xl mx-auto">
    ...
    <div class="flex flex-wrap gap-2 mb-6" id="filtros">
        <button data-filtro="todos" class="filtroBtn px-4 py-2 rounded-xl text-sm font-semibold bg-[#4A9FCA] text-white">
            Todos
        </button>
        <button data-filtro="solicitado" class="filtroBtn px-4 py-2 rounded-xl text-sm font-semibold bg-white text-gray-600 border border-gray-200 hover:bg-gray-50">
            Solicitadas
        </button>
        <button data-filtro="em_analise" class="filtroBtn px-4 py-2 rounded-xl text-sm font-semibold bg-white text-gray-600 border border-gray-200 hover:bg-gray-50">
            Em análise
        </button>
        <button data-filtro="assinado" class="filtroBtn px-4 py-2 rounded-xl text-sm font-semibold bg-white text-gray-600 border border-gray-200 hover:bg-gray-50">
            Assinadas
        </button>
        <button data-filtro="erro" class="filtroBtn px-4 py-2 rounded-xl text-sm font-semibold bg-white text-gray-600 border border-gray-200 hover:bg-gray-50">
            Com erro
        </button>
    </div>
    ...
    <p id="msgVazioFiltro" class="hidden text-center text-gray-400 italic py-10">Nenhuma solicitação nesse status.</p>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-5" id="formAssinado">
            ...
            <div>
                ...
                <!-- Campos reais enviados ao servidor (ocultos, preenchidos via JS) -->
...
// Filtro por status
const filtroBtns = document.querySelectorAll('.filtroBtn');
const cards = document.querySelectorAll('.card-assinatura');
const msgVazio = document.getElementById('msgVazioFiltro');

filtroBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        filtroBtns.forEach(b => {
            b.classList.remove('bg-[#4A9FCA]', 'text-white');
            b.classList.add('bg-white', 'text-gray-600', 'border', 'border-gray-200');
        });
        btn.classList.add('bg-[#4A9FCA]', 'text-white');
        btn.classList.remove('bg-white', 'text-gray-600', 'border', 'border-gray-200');

        const filtro = btn.dataset.filtro;
        let visiveis = 0;
        cards.forEach(card => {
            const mostrar = filtro === 'todos' || card.dataset.status === filtro;
            card.classList.toggle('hidden', !mostrar);
            if (mostrar) visiveis++;
        });
        msgVazio.classList.toggle('hidden', visiveis > 0 || cards.length === 0);
    });
});

/* =========================================================================
 * UPLOAD ZONE UNICA (aceita ate 2 arquivos), distribuindo automaticamente
 * para os campos ocultos assinado1 (obrigatorio) e assinado2 (opcional)
 * que o PHP ja espera receber. Nenhuma logica de back-end precisou mudar.
 * ========================================================================= */
...
inputMultiAssinado.addEventListener('change', () => {
    ...
    inputMultiAssinado.value = ''; // permite selecionar o mesmo arquivo novamente depois
});
...
// Validacao antes de enviar: exige pelo menos 1 arquivo
...
// Reseta a area de upload sempre que o modal for reaberto
